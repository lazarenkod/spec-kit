...
- **[COMP:COMP-001]**: Component creation task links to Component Registry (for UI features)
- **[WIRE:COMP-001→SCR-001]**: Wiring task connecting component to screen (for UI features)
- **[SCREEN:SCR-001]**: Screen implementation task links to Screen Registry (for UI features)
...
### Component Integration Marker Examples *(for UI features with reusable components)*

```markdown
# Component creation task - creates reusable UI component
# Must link to Component Registry in spec.md (COMP-xxx)
- [ ] T050 [P] [US1] [COMP:COMP-001] [VR:VR-001] Create FontSizeSliderView in shared/ui/controls/FontSizeSliderView.kt
  - **Target Screens**: Settings, ReaderOverlay (from Component Registry)
  - **Type**: Control (interactive element)

# Screen implementation task - creates navigation screen
# Must link to Screen Registry in spec.md (SCR-xxx)
- [ ] T055 [US2] [SCREEN:SCR-001] [DEP:T040] Create SettingsScreen in app/screens/SettingsScreen.kt
  - **Route**: /settings (Tab in MainNavigation)
  - **Required Components**: COMP-001, COMP-002 (from Screen Registry)

# Component wiring task - integrates component INTO screen
# CRITICAL: This ensures components are actually USED in their target screens
# QG-COMP-002 validates every (Component, Target Screen) pair has a wire task
- [ ] T060 [WIRE:COMP-001→SCR-001] [DEP:T050,T055] Wire FontSizeSliderView into SettingsScreen
  - **Component File**: shared/ui/controls/FontSizeSliderView.kt
  - **Screen File**: app/screens/SettingsScreen.kt
  - **Action**: Import component, replace placeholder
  - **Verify**: Component renders, is functional in Settings

# Multiple components wired to same screen
- [ ] T061 [WIRE:COMP-002→SCR-001] [DEP:T051,T055] Wire ThemeSwitcherView into SettingsScreen
- [ ] T062 [WIRE:COMP-001→SCR-002] [DEP:T050,T056] Wire FontSizeSliderView into ReaderOverlay
- [ ] T063 [WIRE:COMP-002→SCR-002] [DEP:T051,T056] Wire ThemeSwitcherView into ReaderOverlay
```

**⚠️ WHY WIRE TASKS MATTER**: Without explicit wire tasks, components get created but never integrated into all target screens. The component exists, tests pass, but navigation shows placeholder `Text()` instead of real component. Wire tasks close this gap.
...
<!--
  ENHANCED TEST TASK FORMAT:
  Generated by /speckit.tasks with detailed test structure, data suggestions, and edge cases.
  Format includes:
  - Test type classification (Integration, Unit, Contract, Security)
  - ARRANGE/ACT/ASSERT structure mapped from Given/When/Then
  - Test data suggestions (language-specific)
  - Edge case test tasks with [TEST:AS-xxx:EDGE-n] markers
  - Property-based testing suggestions (when applicable)
-->

#### Test: Integration Test - [Scenario Summary]

- [ ] T010 [P] [US1] [TEST:AS-1A] Integration Test: [scenario description]

**Scenario**: AS-1A - [Given/When/Then description]

**Test Structure**:
```[language]
# ARRANGE (Given: [preconditions])
[setup_code_suggestions - e.g., create test data, configure mocks]

# ACT (When: [action])
[action_code_suggestion - e.g., API call, function invocation]

# ASSERT (Then: [expected_outcome])
[assertion_code_suggestions - e.g., status code, response validation, side effects]
```

**Test Data Suggestions**:
- [entity_1]: [language-specific suggestion - e.g., "Use faker.email() for emails"]
- [entity_2]: [language-specific suggestion - e.g., "Use helper: generate_secure_password()"]
- [entity_3]: [language-specific suggestion - e.g., "Mock external service: @mock.patch('service.call')"]

**Edge Cases to Cover**: [See edge case tests below if applicable, or "None - covered by AS-1B"]

**Estimated Effort**: [2-4 hours including test setup]

---

#### Test: Unit Test - [Error Path Scenario]

- [ ] T011 [P] [US1] [TEST:AS-1B] Unit Test: [error scenario description]

**Scenario**: AS-1B - [Given/When/Then description]

**Test Structure**:
```[language]
# ARRANGE (Given: [error_condition])
[error_condition_setup - e.g., invalid input, missing data]

# ACT (When: [action])
[action_that_triggers_error]

# ASSERT (Then: [expected_error_behavior])
[error_assertions - e.g., status code 400, specific error message]
```

**Test Data Suggestions**:
- [entity]: [suggestion for invalid data - e.g., "Use 'invalid@' for malformed email"]

**Estimated Effort**: [30-60 minutes]

---

#### Edge Case Tests (if suggested_edge_cases present in spec.md)

<!--
  Generated from section 1.1's suggested_edge_cases field.
  Each edge case gets dedicated test task with [TEST:AS-xxx:EDGE-n] marker.
-->

- [ ] T012 [P] [US1] [TEST:AS-1A:EDGE-1] **Edge case**: [edge_case_description]

**Given**: [specific_edge_case_condition]
**When**: [action]
**Then**: [expected_error_or_boundary_behavior]

**Test file**: tests/[type]/test_[name].[ext]::[test_function_name]

**Estimated Effort**: [20-30 minutes]

---

#### Property-Based Testing Suggestion (if applicable)

<!--
  Generated when scenario has BOUNDARY classification + numeric/string entities,
  or SECURITY classification + user input.
-->

💡 **Consider Property-Based Testing**: Run `/speckit.properties` for comprehensive testing

**Property**: [property_description - e.g., "All valid emails per RFC 5322 should pass"]
**Framework**: [Hypothesis|fast-check|rapid|jqwik|kotest (based on project language)]
...
- [ ] T024 [P] [US3] [TEST:AS-3A] Integration Test: [scenario description]

**Scenario**: AS-3A - [Given/When/Then description]
...
## Component-Screen Integration Matrix (CSIM) *(for UI features)*

<!--
  CSIM ensures every reusable component is wired into ALL its target screens.

  Problem this solves: Components are created (task complete ✅) but never
  integrated into navigation screens. Code compiles, tests pass, but users
  see placeholder `Text()` instead of real component.

  Quality Gates:
  - QG-COMP-002: Every (Component, Target Screen) pair must have a wire task
  - QG-COMP-004: Post-implement validation - component actually imported/used

  Auto-generated by /speckit.tasks Step 4.5 from Component Registry + Screen Registry.
-->

| COMP ID | Component | SCR ID | Screen | Wire Task | DEP | Status |
|---------|-----------|--------|--------|-----------|-----|--------|
| COMP-001 | [FontSizeSliderView] | SCR-001 | [Settings] | T060 | T050,T055 | [ ] |
| COMP-001 | [FontSizeSliderView] | SCR-002 | [ReaderOverlay] | T062 | T050,T056 | [ ] |
| COMP-002 | [ThemeSwitcherView] | SCR-001 | [Settings] | T061 | T051,T055 | [ ] |
| COMP-002 | [ThemeSwitcherView] | SCR-002 | [ReaderOverlay] | T063 | T051,T056 | [ ] |

### CSIM Coverage Metrics
...
| Total Screens | 2 | - |
| Total Wire Pairs | 4 | - |
| Wire Tasks Created | 4 | - |
| **Coverage** | - | **100%** |

**QG-COMP-002 Validation**: ✅ All (Component, Target Screen) pairs have wire tasks

### Orphan Detection *(post-implement)*

<!--
  QG-COMP-004 validates that completed wire tasks actually result in:
  1. Import statement in screen file
  2. Component usage in render/compose function

  Run by /speckit.analyze in QA mode.
-->

| Wire Task | Component File | Screen File | Import Found | Usage Found | Status |
|-----------|----------------|-------------|--------------|-------------|--------|
| T060 | shared/ui/.../FontSizeSliderView | app/screens/SettingsScreen | [ ] | [ ] | ⬜ |
| T061 | shared/ui/.../ThemeSwitcherView | app/screens/SettingsScreen | [ ] | [ ] | ⬜ |
| T062 | shared/ui/.../FontSizeSliderView | app/screens/ReaderOverlay | [ ] | [ ] | ⬜ |
| T063 | shared/ui/.../ThemeSwitcherView | app/screens/ReaderOverlay | [ ] | [ ] | ⬜ |

**⚠️ Placeholder Detection**: Screens with Required Components should NOT contain:
- `Text("Settings")` or similar placeholder text
- `// TODO: Add component`
...
> Generated by `/speckit.properties`. Execute after unit tests pass.

| Task ID | Property | Type | Language | Generator | Depends On | Status |
|---------|----------|------|----------|-----------|------------|--------|
| TASK-PBT-001 | PROP-001 | inverse | Python | valid_user | TASK-xxx | ⬜ |
| TASK-PBT-002 | PROP-002 | boundary | TypeScript | invalid_email | TASK-xxx | ⬜ |
| TASK-PBT-003 | PROP-003 | invariant | Go | any_input | TASK-xxx | ⬜ |

### PBT Execution Order

1. **Generator Setup**: Create custom arbitraries/generators for domain entities
2. **Inverse Properties**: Test round-trip operations first (highest signal)
3. **Invariant Properties**: Verify system constraints hold
4. **Boundary Properties**: Test edge cases and error handling
5. **Security Properties**: Verify injection resistance (SQL, XSS, path traversal)
6. **Model-Based Tests**: Run stateful tests last (most complex)

### PBT Commands by Language

| Language | Run Command | Coverage |
|----------|-------------|----------|
| Python | `pytest tests/properties/ --hypothesis-show-statistics` | `--cov` |
| TypeScript | `npm run test:properties` | `--coverage` |
| Go | `go test -v ./... -run TestProperty` | `-cover` |
| Java | `mvn test -Dtest=*Property*` | `jacoco:report` |
| Kotlin | `gradle test --tests '*Property*'` | `kover` |
...
- **[COMP:COMP-001]** = component creation task links to Component Registry (QG-COMP-001 validates)
- **[WIRE:COMP-001→SCR-001]** = wiring task ensures component is integrated into screen (QG-COMP-002 validates)
- **[SCREEN:SCR-001]** = screen implementation task links to Screen Registry
...
- For UI features: Ensure every component has wire tasks for ALL target screens (CSIM 100% coverage)
- For UI features: Run QG-COMP-004 validation to detect orphan components (created but not used)
